...
public class OneWayTrip_test extends TestBase {

	@Test(priority = 2, dataProvider = "One_Way_data")
	public void oneWayTripFlight(String username, String password, String passcount, String departfrom,
		...
		try {
			extenttest = extent.startTest("One Way Trip");
			logger.info("Starting One Way Trip TestCase");
			extenttest.log(LogStatus.INFO, "Starting One Way Trip TestCase");
			LoginPage loginPage = new LoginPage();
			loginPage.signIn(username, password);
			Screenshot.takeShot("Sign In Successful", driver);
			FlightFinder flightfinder = new FlightFinder();
			flightfinder.oneWayFlightFinder(passcount, departfrom, departmonth, departday, arriveto, returnmonth,
			...
			Screenshot.takeShot("Confirm", TestBase.driver);
			...
			ScreenshotExtentReport.captureExtentRepScreenshotPass("One Way Trip TestCase Passed");
			...
			logger.info("One Way Trip TestCase Passed");
			extent.endTest(extenttest);
		} catch (Error e) {
			logger.error(e.getMessage());
			extenttest.log(LogStatus.ERROR, e.getMessage());
			ScreenshotExtentReport.captureExtentRepScreenshotFail("One Way Trip TestCase Failed");
			extent.endTest(extenttest);
	...
	@DataProvider(name = "One_Way_data")
	public Object[][] onewayData() {
		Object[][] data = null;
		try {
			...
			int rowcount = exceldata.getRowCount("One Way Trip");
			int colcount = exceldata.getColCount("One Way Trip");
			data = new Object[rowcount][colcount];
			for (int i = 0; i < rowcount; i++) {
				for (int j = 0; j < colcount; j++) {
					data[i][j] = exceldata.getData("One Way Trip", i + 1, j);
				}
			}
